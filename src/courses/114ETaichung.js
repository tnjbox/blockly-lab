...
const course = //
// 2026-09-25：114ETaichung-5（提前結束）的starterXml已改寫，把原本「重複N次事件裡用
// controls_flow_statements(BREAK)在紅牌觸發時中斷循環」的寫法，改成「重複N次事件，
// 每次事件處理前先判斷『尚未觸發任一張紅牌』（旗標變數，即not(redA or redB)）才處理
// 這筆事件，觸發後的事件全部略過」。兩種寫法在功能上完全等價（已用原本4筆測資逐筆
// 驗證輸出一致），改寫原因：Scratch平台的xml-to-scratch.js轉換器不支援break/continue
// （scratch-vm沒有對應的執行語意，詳見osep-judge對話記錄的技術評估），這是唯一一處
// 用到break的題目，改寫後這題終於能自動轉換出Scratch示範解答。
{
  "code": "114ETaichung",
  "title": "114-臺中市國小",
  "type": "programming",
